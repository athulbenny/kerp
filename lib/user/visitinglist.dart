...
class VisitingList extends StatefulWidget {
  const VisitingList({required this.pid,required this.isVisited});
  static final String id="visitinglist";
  final String pid;final int isVisited;
  @override
  State<VisitingList> createState() => _VisitingListState(pid,isVisited);
}

class _VisitingListState extends State<VisitingList> {
  String pid;int isVisited;_VisitingListState(this.pid,this.isVisited);
  ...
  Future<dynamic> generateUserList() async {
    ...
    List<Employee> _employees = await list.map<Employee>((json) => Employee.fromJson(json)).toList();
    return _employees;
  }
  @override
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.connectionState == ConnectionState.done) {
                      if (snapshot.hasError) {
                        return const Center(
                          child: Text(
                            'Location not assigned by admin',
                        ...
                        return Container(
                          ...
                          SizedBox(height: MediaQuery.of(context).size.height/80,child: Container(),),
                            ...
                            const [Text('location', style: TextStyle(color:Colors.white,fontSize: 20.0))])),
                            const Expanded(flex:1,child: SizedBox(width:1)),
                            Expanded(flex:2,child: Column(children:
                            const [Text('latitude', style: TextStyle(color:Colors.green,fontSize: 20.0))])),
                            const Expanded(flex:1,child: SizedBox(width: 5,)),
                            Expanded(flex:2,child: Column(children:
                            const [Text('longitude', style: TextStyle(color:Colors.green,fontSize: 20.0))])),
                              const Expanded(flex:1,child: SizedBox(width: 5,)),
                              Expanded(flex:2,child: Column(children:
                              const [Text('Date', style: TextStyle(color:Colors.white,fontSize: 20.0))])),
                            ]),
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Row( children: [
                        Expanded(flex:3,child: Container(padding:EdgeInsets.only(left: MediaQuery.of(context).size.height/100),child:
                          Text(loc[data], style: TextStyle(fontSize: 20.0)))),
                        //Expanded(flex:0,child: SizedBox(width:5,child: Container(color: Colors.black,),)),
                        Expanded(flex:2,child: Container(child:
                          Text(lat[data].toString(), style: TextStyle(color:Colors.green,fontSize: 20.0)))),
                        //Expanded(flex:0,child: SizedBox(width: 5,child: Container(color: Colors.black,),)),
                        Expanded(flex:2,child: Container(child:
                          Text(long[data].toString(), style: TextStyle(color:Colors.green,fontSize: 20.0)))),
                        //Expanded(flex:0,child: SizedBox(width: 5,child: Container(color: Colors.black,),)),
                        Expanded(flex:2,child: Container(child:
                          Text(tdate[data].toString(), style: TextStyle(fontSize: 20.0)))),
                      ]),
                    ),//SizedBox(height: 2,child: Container(color: Colors.black,),),
                  ],
                ),
              );
            },separatorBuilder: (context,index){return Container(height: MediaQuery.of(context).size.height/65,);},
          ),
    );
  }
}